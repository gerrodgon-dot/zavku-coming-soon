...
test('missing delivery config never reports success',async t=>{const post=await fixture(t,{deliver:null});assert.equal((await post()).status,503);});
...
test('rate limit applies even when caller spoofs forwarded-for',async t=>{const post=await fixture(t);for(let i=0;i<5;i++)assert.equal((await post(good,{'X-Forwarded-For':`192.0.2.${i}`})).status,503);const response=await post();assert.equal(response.status,429);assert.ok(response.headers.has('retry-after'));});
